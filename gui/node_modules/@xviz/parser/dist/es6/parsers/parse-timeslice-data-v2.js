...
export default function parseStreamSet(data, convertPrimitive) {
  ...
  if (update_type !== 'snapshot') {
    log.error(`Only XVIZ update_type of "snapshot" is currently supported. Type "${update_type}" is not supported.`)();
    return {
  ...
  if (updates.length > 1) {
    log.warn(`Only XVIZ first update of "snapshot" is currently supported. Current updates has "${updates.length}" entries.`)();
  }
  ...
  const result = {
    ...
    streams: newStreams,
    timestamp // TODO/Xintong validate primary vehicle pose in each update?
