...
export function parseStreamPrimitive(primitives, streamName, time, convertPrimitive) {
  ...
  const primType = primitiveData.type,
        objects = primitiveData.primitives;
  const primitiveMap = createPrimitiveMap();
  let category = null; // Primitives are an array of XVIZ objects

  for (let objectIndex = 0; objectIndex < objects.length; objectIndex++) {
    const object = objects[objectIndex]; // array of primitives
    ...
    if (object && Array.isArray(object)) {
      ...
      const primitive = normalizeXVIZPrimitive(PRIMITIVE_SETTINGS, object, objectIndex, streamName, primType, time, convertPrimitive); // Allow for v1 inline type to override primitive type
      ...
      if (primitive) {
        ...
        if (isMainThread && ( // OBJECT_STREAM is deprecated, only keeping for backward compatibility
        streamName === OBJECT_STREAM || !OBJECT_STREAM && primitive.id && category === 'features')) {
...
export function parseStreamFuturesV1(objects, streamName, time, convertPrimitive) {
  const futures = []; // objects = array of objects
  ...
  objects.forEach((object, objectIndex) => {
    const primitives = object.primitives;
...
export function parseStreamTimeSeries(seriesArray, streamBlackList) {
  const _getXVIZConfig4 = getXVIZConfig(),
...
function parseStreamTimeSeriesV2(seriesArray, streamBlackList) {
  ...
  seriesArray.forEach(timeSeriesEntry => {
    const timestamp = timeSeriesEntry.timestamp,
...
const DEFAULT_COLOR = [0, 0, 0, 255]; // Joins a set of point clouds extracted from objects into a single point cloud
...
function joinObjectPointCloudsToTypedArrays(objects) {
  ...
  const colors = new Uint8ClampedArray(numInstances * 4); // Store object ids to enable recoloring.
  ...
  objects.forEach(object => {
    const vertexPositions = object.vertices; // object.color is V1 and should be removed when deprecated
