...
import { parseLogMetadata } from './parse-log-metadata'; // Handle messages from the stand alone video server
...
export function parseVideoFrame(arrayBuffer) {
  const view = new DataView(arrayBuffer); // Read off version

  const result = {
    type: LOG_STREAM_MESSAGE.VIDEO_FRAME
  };
  const littleEndian = true;
  const utf8Decoder = new TextDecoder('utf-8'); // Check version
