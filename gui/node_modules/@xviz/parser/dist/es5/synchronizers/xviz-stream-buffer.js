...
var RIGHT = _search.INSERT_POSITION.RIGHT; // Buffer types
  ...
  _createClass(XVIZStreamBuffer, [{
    ...
    value: function insert(timeslice) {
      ...
      var _loop = function _loop(streamName) {
        _this.streams[streamName] = timeslices.map(function (timeSlice) {
          return timeSlice.streams[streamName];
        }).filter(Boolean);
      };
      ...
      var _loop2 = function _loop2(streamName) {
        _this.videos[streamName] = timeslices.map(function (timeSlice) {
          return timeSlice.videos && timeSlice.videos[streamName];
        }).filter(Boolean);
      };

      for (var streamName in timeslice.videos) {
        _loop2(streamName);
      }
