...
function (_XVIZWebsocketLoader) {
  ...
  function XVIZStreamLoader() {
    ...
    _defineProperty(_assertThisInitialized(_assertThisInitialized(_this)), "_onXVIZTimeslice", function (message, bufferUpdated) {
      if (bufferUpdated) {
        _this.bufferRange = rangeUtils.add([_this.lastRequest.startTimestamp, message.timestamp], _this.bufferRange);
      }
    });
  ...
  _createClass(XVIZStreamLoader, [{
    ...
    value: function seek(timestamp) {
      ...
      var oldVersion = this.streamBuffer.valueOf();
