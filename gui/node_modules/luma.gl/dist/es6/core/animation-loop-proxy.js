...
export default class AnimationLoopProxy {
  ...
  _createAndTransferCanvas(opts) {
    ...
    const screenCanvas = getCanvas(opts); // Create an offscreen canvas controlling the main canvas

    if (!screenCanvas.transferControlToOffscreen) {
      log.error('OffscreenCanvas is not available in your browser.')(); // eslint-disable-line
    }

    const offscreenCanvas = screenCanvas.transferControlToOffscreen(); // Transfer the offscreen canvas to the worker
