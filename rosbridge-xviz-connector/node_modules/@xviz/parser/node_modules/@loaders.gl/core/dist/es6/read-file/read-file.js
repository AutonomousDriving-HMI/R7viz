...
export function readFile(uri) {
  ...
  try {
    ...
    return fetch(uri, options).then(res => res[options.dataType]()); // return Promise.reject(new Error('Cannot load file URIs in browser'));
  } catch (error) {
...
function readFileNode(filename, options) {
  ...
  return readFileAsync(filename, options).then(buffer => buffer instanceof Buffer ? toArrayBuffer(buffer) : buffer);
}
