...
import assert from '../utils/assert'; // TODO: support progress and abort
// TODO: support moving loading to worker

export function parseWithLoader(data, loader) {
  let options = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : {};
  let url = arguments.length > 3 ? arguments[3] : undefined;
  validateLoaderObject(loader); // Normalize options
  ...
  if (loader.parse) {
    const promise = loader.parse(data, options); // NOTE: keep return on separate statement to facilitate breakpoints here when debugging
...
function promisify(parserFunc, loader, url, data, options) {
  return new Promise((resolve, reject) => {
    try {
      const result = resolve(parserFunc(data, options)); // NOTE: return on separate statement to facilitate breakpoint setting here when debugging
...
function validateLoaderObject(loader) {
  const hasParser = loader.parseTextSync || loader.parseText || loader.parseSync || loader.parse || loader.loadAndParse || loader.parseStream || loader.worker;
  assert(hasParser);
