...
function getPngSize(contents) {
  var dataView = toDataView(contents); // Check file contains the first 4 bytes of the PNG signature.
...
function getGifSize(contents) {
  var dataView = toDataView(contents); // Check first 4 bytes of the GIF signature ("GIF8").
...
function getBmpSize(contents) {
  var dataView = toDataView(contents); // Check magic number is valid (first 2 characters should be "BM").
...
function getJpegSize(contents) {
  var dataView = toDataView(contents); // Check file contains the JPEG "start of image" (SOI) marker.
