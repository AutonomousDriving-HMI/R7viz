...
function getPngSize(contents) {
  const dataView = toDataView(contents); // Check file contains the first 4 bytes of the PNG signature.
...
function getGifSize(contents) {
  const dataView = toDataView(contents); // Check first 4 bytes of the GIF signature ("GIF8").
...
function getBmpSize(contents) {
  const dataView = toDataView(contents); // Check magic number is valid (first 2 characters should be "BM").
...
function getJpegSize(contents) {
  const dataView = toDataView(contents); // Check file contains the JPEG "start of image" (SOI) marker.
  ...
  while (i < dataView.byteLength) {
    const marker = dataView.getUint16(i, BIG_ENDIAN); // The frame that contains the width and height of the JPEG image.
