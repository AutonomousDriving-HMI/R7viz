...
export default class GLTFParser {
  ...
  parse(gltf) {
    let optionsParam = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : {};
    const options = Object.assign({}, this.options, optionsParam); // GLTF can be JSON or binary (GLB)
    ...
    if (!this.glbParser) {
      this.gltf.buffers.forEach(buffer => {
        ...
        if (buffer.uri) {
          ...
          promises.push(window.fetch(this._getFullUri(buffer.uri, options.uri)).then(res => res.arrayBuffer()).then(data => {
            buffer.data = data;
          }));
        }
      });
  ...
  _decompressUberDracoPointCloud(primitive, extensions) {
    ...
    const bufferView = this.getBufferView(bufferViewIndex); // TODO: change to getArrayFromBufferView()

    const compressedData = this.glbParser.getBufferView(bufferView);
    const dracoDecoder = new this.DracoDecoder();
    const decodedPrimitive = dracoDecoder.decode(compressedData); // TODO: what to do about original attributes
