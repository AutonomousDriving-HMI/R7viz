...
import { getXVIZConfig } from '../config/xviz-config'; // returns true if the input represents a JSON string.
...
const XVIZ_TYPE_PATTERN = /"type":\s*"xviz\//; // returns true if the input represents an enveloped XVIZ object as a JSON string.
// Can be either string or Uint8Array

function isXVIZJSONString(str) {
  // {"type":"xviz/
  let firstChunk = str.slice(0, 14); // "type":"xviz/*"}
...
export function parseStreamDataMessage(message, onResult, onError, opts) {
  ...
  try {
    ...
    if (parseData) {
      const result = parseStreamLogData(data, _objectSpread({}, opts, {
        v2Type
      }));
      onResult(result);
    }
...
export function parseStreamLogData(data) {
  ...
  switch (typeKey) {
    ...
    case 'metadata':
      return _objectSpread({}, parseLogMetadata(data), {
    ...
    case 'transform_log_done':
      return _objectSpread({}, data, {
        type: LOG_STREAM_MESSAGE.DONE
      });

    case 'error':
      return _objectSpread({}, data, {
        message: 'Stream server error',
        type: LOG_STREAM_MESSAGE.ERROR
      });
    // v1 types
