...
import { getTransformsFromPose } from '../parsers/parse-vehicle-pose'; // lookAheads is an array of arrays, so we need to fetch out the first
...
export default class LogSlice {
  ...
  getCurrentFrame(params, postProcessFrame) {
    const vehiclePose = params.vehiclePose;
    ...
    const frame = _objectSpread({}, params, getTransformsFromPose(vehiclePose), {
      vehiclePose,
  ...
  initialize(streamFilter, lookAheadMs, streamsByReverseTime) {
    const filter = streamFilter && Object.keys(streamFilter).length > 0 ? streamFilter : null; // get data if we don't already have that stream && it is not filtered.
  ...
  addStreamDatum(datum, streamName, lookAheadMs) {
    ...
    const _datum$features = datum.features,
