...
export default class LogSynchronizer extends BaseSynchronizer {
   ...
   */
  constructor() {
    let logs = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : {};
    let opts = arguments.length > 1 ? arguments[1] : undefined;
    super(opts); // Set up log state for all logs, so that we can move forward and back
  ...
  _getTimeRangeInReverse(startTime, endTime) {
    const streams = {}; // Set index based on time range for each stream
  ...
  _lookupStreamDatum(logName, startTime, endTime) {
    ...
    assert(log, 'Invalid log'); // This is an optimization for positive time deltas (playing forward)
    ...
    let endIndex = null;
    let endTimestamp; // invalidate

    log.index = null; // Find the range of indices for the given start and end time

    for (let i = startIndex; i < log.data.length; ++i) {
      const timestamp = this._getTimeFromObject(log.data[i]); // If timestamp < startTime, sample before our target window, so don't update index
