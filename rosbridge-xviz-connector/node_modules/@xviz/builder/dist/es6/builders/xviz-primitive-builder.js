...
export default class XVIZPrimitiveBuilder extends XVIZBaseBuilder {
  constructor(props) {
    ...
    this.reset(); // primitives: {[streamId]: []}
  ...
  stadium(start, end, radius) {
    ...
    if (start.length !== 3) {
      this.validateError(`The start position must be of the form [x, y, z] where ${start} was provided`);
    }

    if (end.length !== 3) {
      this.validateError(`The end position must be of the form [x, y, z] where ${end} was provided`);
    }
  ...
  _flushPrimitives() {
    ...
    const primitive = this._formatPrimitive(); // Each type like "image" has an "images" array, this hack saves a
    // big switch statement.


    const arrayFieldName = `${this._type}s`;
    let array = stream[arrayFieldName]; // Make sure array exists
