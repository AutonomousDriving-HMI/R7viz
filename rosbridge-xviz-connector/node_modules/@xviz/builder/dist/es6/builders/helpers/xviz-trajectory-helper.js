...
export function getObjectTrajectory(_ref2) {
  let targetObject = _ref2.targetObject,
  ...
  for (let i = 0; i < motions.length; i++) {
    ...
    const _getGeospatialVector = getGeospatialVector(startVehiclePose, currVehiclePose, startVehiclePose.yaw),
          _getGeospatialVector2 = _slicedToArray(_getGeospatialVector, 2),
          x = _getGeospatialVector2[0],
          y = _getGeospatialVector2[1];

    const transformMatrix = new Pose(currVehiclePose).getTransformationMatrixFromPose(new Pose(startVehiclePose)); // objects in curr frame are meters offset based on current vehicle pose
...
export function getGeospatialVector(from, to) {
  ...
  const distInMeters = turf.distance(fromCoord, toCoord, {
    units: 'meters'
  }); // Bearing is degrees from north, positive is clockwise

  const bearing = turf.bearing(fromCoord, toCoord); // Get the bearing relative to heading
