...
export function getImageSize(contents, mimeType) {
  const ERR_INVALID_TYPE = `Invalid MIME type. Supported MIME types are: ${Array.from(mimeTypeMap.keys()).join(', ')}`; // Looking for only a specific MIME type.
...
function getJpegSize(contents) {
  ...
  while (i < contents.length) {
    ...
    if (sofMarkers.has(marker)) {
      // Number of lines.
      const height = contents.readUInt16BE(i + 5); // Number of pixels per line.
