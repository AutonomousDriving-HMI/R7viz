...
export function encodeBinaryXVIZ(xvizJson, options) {
  const gltfBuilder = new GLTFBuilder(options); // Pack appropriate large data elements (point clouds and images) in binary

  const packedData = packBinaryJson(xvizJson, gltfBuilder, null, options); // As permitted by glTF, we put all XVIZ data in a top-level subfield.
...
export function writeBinaryXVIZtoFile(sink, directory, name, json, options) {
  ...
  sink.writeSync(directory, `${name}.glb`, toBuffer(glbFileBuffer), {
    flag: 'w'
  });
