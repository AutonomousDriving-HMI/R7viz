...
function getImageSize(contents, mimeType) {
  var ERR_INVALID_TYPE = "Invalid MIME type. Supported MIME types are: ".concat(Array.from(mimeTypeMap.keys()).join(', ')); // Looking for only a specific MIME type.
...
function getJpegSize(contents) {
  ...
  while (i < contents.length) {
    ...
    if (sofMarkers.has(marker)) {
      // Number of lines.
      var height = contents.readUInt16BE(i + 5); // Number of pixels per line.
